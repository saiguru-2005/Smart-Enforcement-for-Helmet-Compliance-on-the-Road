...
def detectObject(CNNnet, total_layer_names, image_height, image_width, image, name_colors, class_labels, indexno, detect=True):
    try:
        if detect:
            ...
            if Boundingboxes is None or confidence_value is None or class_ids is None:
                raise ValueError('Bounding boxes, confidence values, or class IDs are None.')
            ids = cv.dnn.NMSBoxes(Boundingboxes, confidence_value, 0.5, 0.3)
            if ids is None:
                raise ValueError('IDs are None after NMSBoxes operation.')
